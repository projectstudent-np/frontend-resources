...
export default function ProfileSlideout({ open, onClose }: ProfileSlideoutProps) {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            ...
            const statusLocked = !!student.status && student.status !== 'draft' && student.status !== 'info_requested';
    ...
    return createPortal(
        ...
        >
            ...
            >
                ...
                <div className="slideout-header">
                    <div className="slideout-avatar-area">
                        ...
                        >
                            {avatarUrl ? (
                                <img src={avatarUrl} alt="Foto do perfil" className="slideout-avatar-img" />
                            ) : (
                                <svg width="96" height="96" viewBox="0 0 48 48" fill="none">
                                    <circle cx="24" cy="24" r="24" fill="#F2F4F7" />
                                    <circle cx="24" cy="19" r="8" fill="#98A2B3" />
                                    <path d="M10 42c0-8 6-14 14-14s14 6 14 14" stroke="#98A2B3" strokeWidth="2.5" fill="none" />
                                </svg>
                            )}
                            {canEditAvatar && (
                        ...
                        <div className="slideout-user-info">
                            <div className="slideout-user-name-row">
                                <span className="slideout-user-name">{user.full_name || 'Usuário'}</span>
                                <span className={`badge ${ROLE_BADGES[user.role]}`}>{ROLE_LABELS[user.role]}</span>
                            </div>
                            <span className="slideout-user-cpf">{formatCPFDisplay(user.cpf)}</span>
                        </div>
                    </div>
                    ...
                    <div className="slideout-stats">
                        <div className="slideout-stat">
                            ...
                            <span className="slideout-stat-value">{ROLE_LABELS[user.role] ?? user.role}</span>
                        </div>
                        <div className="slideout-stat">
                            <span className="slideout-stat-label">Conta criada em</span>
                            <span className="slideout-stat-value">{formatDate(user.created_at)}</span>
                        </div>
                        <div className="slideout-stat">
                            <span className="slideout-stat-label">CPF</span>
                            <span className="slideout-stat-value">{formatCPFDisplay(user.cpf)}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="slideout-body">
                    ...
                    <form id="slideout-profile-form" onSubmit={handleSubmit} className="slideout-form" noValidate>
                        <div className="slideout-separator" />

                        <div className="slideout-field-row">
                            <div className="input-group">
                                <label className="input-label" htmlFor="slideout-name">Nome completo</label>
                                <input
                                    id="slideout-name"
                                    type="text"
                                    ...
                                    onChange={(e) => { setFullName(toTitleCase(e.target.value)); setSuccess(''); }}
                                />
                            </div>
                        </div>
                        ...
                        <div className="input-group">
                            <label className="input-label" htmlFor="slideout-email">Email</label>
                            <input
                                id="slideout-email"
                                type="email"
                                className={`input-field${fieldsLocked ? ' input-locked' : ''}`}
                        ...
                        <div className="input-group">
                            <label className="input-label" htmlFor="slideout-phone">Telefone</label>
                            <input
                                id="slideout-phone"
                                type="text"
                                inputMode="numeric"
                                ...
                                maxLength={15}
                                disabled={fieldsLocked}
                                onChange={(e) => { setPhone(e.target.value.replace(/\D/g, '')); setSuccess(''); }}
                            />
                        </div>
                        ...
                        >
                            ...
                            {sendingReset ? 'Enviando...' : 'Enviar email de alteração de senha'}
                        </button>
                    </form>
                </div>
            ...
            {/* Confirm modal */}
            {confirmOpen && (
                <div className="slideout-confirm-overlay" onClick={(e) => e.stopPropagation()}>
                    <div className="profile-confirm-modal card" onClick={(e) => e.stopPropagation()}>
                        ...
                        <p className="profile-confirm-desc">
                            Você confirma que os dados digitados estão corretos? Não será possível alterá-los enquanto a carteirinha estiver em processo de análise.
                        </p>
