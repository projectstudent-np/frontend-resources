    ...
    return (
        <div className="stepper">
            {STEPS.map((step, i) => {
                ...
                return (
                    <div key={i} style={{ display: 'flex', alignItems: 'flex-start', flex: i < STEPS.length - 1 ? 1 : '0 0 auto' }}>
                        ...
                        {i < STEPS.length - 1 && (
                            <div className={`stepper-connector ${isCompleted ? 'completed' : 'dashed'}`} />
                        )}
                    </div>
                );
            })}
...
function StepPersonalInfo({ onNext }: { onNext: () => void }) {
    ...
    return (
        <div className="step-card card">
            ...
            <p className="step-card-desc">Preencha seus dados pessoais para continuar.</p>

            <div className="step-form-grid">
                <div className="input-group">
                    <label className="input-label" htmlFor="step-name">Nome completo *</label>
                    <input
                        id="step-name"
                        type="text"
                ...
                <div className="input-group">
                    <label className="input-label" htmlFor="step-phone">Telefone *</label>
                    <input
                        id="step-phone"
                        type="text"
                        inputMode="numeric"
                ...
                <div className="step-readonly">
                    ...
                    <span className="step-readonly-value">{formatDate(user.created_at)}</span>
                </div>
            </div>

            {error && <p className="auth-error" style={{ marginTop: 'var(--space-4)' }}>{error}</p>}
...
function StepEducationalData({
    ...
    onBack: () => void;
    form: { curso_id: string; periodo_id: string; student_id_number: string; instituicao_id: string; cidade_id: string };
    ...
    const canContinue =
        form.curso_id && form.periodo_id && form.instituicao_id && form.cidade_id && form.student_id_number.trim();
        ...
        <div className="step-card card">
            ...
            <div className="step-form-grid">
                <div className="input-group">
                    ...
                    <select id="periodo_id" name="periodo_id" className="input-field" value={form.periodo_id} onChange={handleChange} disabled={saving}>
                ...
                <div className="input-group">
                    ...
                    <select id="curso_id" name="curso_id" className="input-field" value={form.curso_id} onChange={handleChange} disabled={saving}>
                ...
                <div className="input-group">
                    ...
                    <select id="instituicao_id" name="instituicao_id" className="input-field" value={form.instituicao_id} onChange={handleChange} disabled={saving}>
                        <option value="">Selecione uma instituição</option>
                        {instituicoes.map((i) => <option key={i.id} value={i.id}>{i.nome}</option>)}
                    </select>
                </div>
                <div className="input-group">
                    <label className="input-label" htmlFor="cidade_id">Cidade da Faculdade *</label>
                    <select id="cidade_id" name="cidade_id" className="input-field" value={form.cidade_id} onChange={handleChange} disabled={saving}>
                ...
                <div className="input-group">
                    ...
                    <input id="student_id_number" name="student_id_number" type="text" className="input-field" placeholder="Número de matrícula" value={form.student_id_number} onChange={handleChange} disabled={saving} />
                </div>
            </div>
            ...
            <div className="step-actions">
                <button type="button" className="btn btn-secondary" onClick={onBack} disabled={saving}>Voltar</button>
                <button type="button" className="btn btn-primary" onClick={handleNext} disabled={!canContinue || saving}>
                    {saving ? 'Salvando...' : 'Salvar e continuar'}
                </button>
            </div>
        </div>
...
function StepAttachments({
    ...
    const [files, setFiles] = useState<UploadedFiles>(existingFiles ?? { foto_3x4: null, comprovante_matricula: null, documento_identidade: null });
    ...
    useEffect(() => {
        const keys = ['foto_3x4', 'comprovante_matricula', 'documento_identidade'] as const;
        keys.forEach(async (key) => {
            const path = files[key];
            if (path) {
                const { data } = await supabase.storage.from('student-documents').createSignedUrl(path, 600);
                if (data?.signedUrl) setPreviewUrls((prev) => ({ ...prev, [key]: data.signedUrl }));
            }
        });
    }, []);

    const allUploaded = !!files.foto_3x4 && !!files.comprovante_matricula && !!files.documento_identidade;
    ...
    async function handleFileSelect(key: keyof UploadedFiles, file: File) {
        ...
        try {
            ...
            const { data: signedData } = await supabase.storage.from('student-documents').createSignedUrl(path, 600);
            if (signedData?.signedUrl) setPreviewUrls((prev) => ({ ...prev, [key]: signedData.signedUrl }));
        } catch (err: unknown) {
    ...
    function triggerInput(key: keyof UploadedFiles, accept: string) {
        ...
        input.type = 'file';
        input.accept = accept;
        input.onchange = (e) => { const file = (e.target as HTMLInputElement).files?.[0]; if (file) handleFileSelect(key, file); };
        input.click();
    }
    ...
    return (
        <div className="step-card card">
            ...
            <p className="step-card-desc">Envie os documentos necessários para a emissão da carteirinha. <span className="upload-limit-hint">Máximo 5MB por arquivo.</span></p>
            ...
            <div className="step-uploads">
                <div className={`upload-item${files.foto_3x4 ? ' upload-done' : ''}`}>
                    ...
                    <div className="upload-info">
                        ...
                        <p className="upload-desc">Foto recente, de frente, com fundo claro. Sem acessórios que cubram o rosto. Esta será a foto da sua carteirinha.</p>
                        {files.foto_3x4 ? (
                            <span className="upload-btn-row">
                                <span className="upload-btn-success">{CHECK_ICON} Enviado</span>
                                <button type="button" className="upload-btn" disabled={!!uploading} onClick={() => triggerInput('foto_3x4', 'image/jpeg,image/png,image/webp')}>
                                    {uploading === 'foto_3x4' ? 'Enviando...' : <>{UPLOAD_ICON} Substituir arquivo enviado</>}
                                </button>
                            </span>
                        ) : (
                            <button type="button" className="upload-btn" disabled={uploading === 'foto_3x4'} onClick={() => triggerInput('foto_3x4', 'image/jpeg,image/png,image/webp')}>
                                {uploading === 'foto_3x4' ? 'Enviando...' : <>{UPLOAD_ICON} Selecionar foto</>}
                            </button>
                        )}
                        {previewUrls.foto_3x4 && (
                            <div className="upload-preview">
                                <img src={previewUrls.foto_3x4} alt="Preview foto 3x4" className="upload-preview-img" />
                            </div>
                        )}
                    </div>
                    <div className="upload-example">
                        <img src="/foto-3x4-example.png" alt="Exemplo foto 3x4" className="upload-example-img" />
                    </div>
                </div>

                <div className={`upload-item${files.comprovante_matricula ? ' upload-done' : ''}`}>
                    <div className="upload-icon">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    <div className="upload-info">
                        ...
                        <p className="upload-desc">Declaração de matrícula ou comprovante de vínculo com a instituição de ensino.</p>
                        {files.comprovante_matricula ? (
                            <span className="upload-btn-row">
                                <span className="upload-btn-success">{CHECK_ICON} Enviado</span>
                                <button type="button" className="upload-btn" disabled={!!uploading} onClick={() => triggerInput('comprovante_matricula', 'image/jpeg,image/png,image/webp,application/pdf')}>
                                    {uploading === 'comprovante_matricula' ? 'Enviando...' : <>{UPLOAD_ICON} Substituir arquivo enviado</>}
                                </button>
                            </span>
                        ) : (
                            <button type="button" className="upload-btn" disabled={uploading === 'comprovante_matricula'} onClick={() => triggerInput('comprovante_matricula', 'image/jpeg,image/png,image/webp,application/pdf')}>
                                {uploading === 'comprovante_matricula' ? 'Enviando...' : <>{UPLOAD_ICON} Selecionar arquivo</>}
                            </button>
                        )}
                        {previewUrls.comprovante_matricula && (
                            <div className="upload-preview">
                                <img src={previewUrls.comprovante_matricula} alt="Preview comprovante" className="upload-preview-img" />
                            </div>
                        )}
                    </div>
                </div>

                <div className={`upload-item${files.documento_identidade ? ' upload-done' : ''}`}>
                    <div className="upload-icon">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    <div className="upload-info">
                        ...
                        <p className="upload-desc">Documento de identidade (frente e verso) ou conta de água/luz recente.</p>
                        {files.documento_identidade ? (
                            <span className="upload-btn-row">
                                <span className="upload-btn-success">{CHECK_ICON} Enviado</span>
                                <button type="button" className="upload-btn" disabled={!!uploading} onClick={() => triggerInput('documento_identidade', 'image/jpeg,image/png,image/webp,application/pdf')}>
                                    {uploading === 'documento_identidade' ? 'Enviando...' : <>{UPLOAD_ICON} Substituir arquivo enviado</>}
                                </button>
                            </span>
                        ) : (
                            <button type="button" className="upload-btn" disabled={uploading === 'documento_identidade'} onClick={() => triggerInput('documento_identidade', 'image/jpeg,image/png,image/webp,application/pdf')}>
                                {uploading === 'documento_identidade' ? 'Enviando...' : <>{UPLOAD_ICON} Selecionar arquivo</>}
                            </button>
                        )}
                        {previewUrls.documento_identidade && (
                            <div className="upload-preview">
                                <img src={previewUrls.documento_identidade} alt="Preview documento" className="upload-preview-img" />
                            </div>
                        )}
                    </div>
                </div>
            ...
            <div className="step-actions">
                <button type="button" className="btn btn-secondary" onClick={onBack} disabled={!!uploading}>Voltar</button>
                <button type="button" className="btn btn-primary" onClick={() => onSubmit(files)} disabled={!allUploaded || submitting || !!uploading}>
...
export default function StudentDashboard() {
    ...
    const [infoRequestJustification, setInfoRequestJustification] = useState<string | null>(null);
    const [infoRequestExecutive, setInfoRequestExecutive] = useState<string | null>(null);
    // Rejected state
    const [rejectionReason, setRejectionReason] = useState<string | null>(null);
    const [rejectionExecutive, setRejectionExecutive] = useState<string | null>(null);
    ...
    const loadData = async () => {
        ...
        try {
            ...
            if (studentErr) console.warn('[StudentDashboard] students query error:', studentErr.message);
            setStudent(studentData);

            // Se draft ou info_requested, restaurar form e posicionar stepper
            if (studentData?.status === 'draft' || studentData?.status === 'info_requested') {
            ...
            if (studentData?.status === 'info_requested') {
                ...
                if (histEntry?.justification) {
                    ...
                    setInfoRequestExecutive((histEntry.users as unknown as { full_name: string })?.full_name ?? null);
                }
            }
            ...
            if (studentData?.status === 'rejected') {
                ...
                if (histEntry) {
                    ...
                    setRejectionExecutive((histEntry.users as unknown as { full_name: string })?.full_name ?? null);
            ...
            if (studentData?.status === 'approved') {
                const { data: fullData } = await supabase
                    .from('students')
                    .select('cursos(nome), periodos(nome), instituicoes(nome), cidades(nome)')
    ...
    async function handleCancelCard() {
        ...
        try {
            await supabase.from('students').update({ status: 'rejected' }).eq('id', student.id);
            await supabase.from('student_cards').update({ status: 'cancelled' }).eq('student_id', student.id).eq('status', 'active');

            const { data: ticket } = await supabase.from('tickets')
                .select('id').eq('user_id', user.id).eq('type', 'card_creation')
    ...
    async function handleSelfResubmit() {
        ...
        try {
            await supabase.from('students').update({ status: 'info_requested' }).eq('id', student.id);
            window.location.reload();
        } catch { /* silencioso */ } finally {
    ...
    const handleFinalSubmit = async (uploadedFiles: UploadedFiles) => {
        ...
        try {
            ...
            if (uploadedFiles.foto_3x4) {
                await supabase.from('users').update({ avatar_path: uploadedFiles.foto_3x4 }).eq('id', userId);
            }
    ...
    const displayName = user?.full_name?.trim() || session?.user?.email || 'Estudante';
    const showStepper = !student || student.status === 'draft' || student.status === 'info_requested';
        ...
        return (
            <div className="dashboard-page">
                <div className="dashboard-header">
                    <div>
                        <h1 className="dashboard-title">
                            {student?.status === 'info_requested' ? 'Atualizar Solicitação' : 'Solicitar Carteirinha'}
                        </h1>
                ...
                {student?.status === 'info_requested' && infoRequestJustification && (
                    <div className="alert-warning" style={{ marginBottom: 'var(--space-6)' }}>
                        <strong>Informações solicitadas{infoRequestExecutive ? ` por ${infoRequestExecutive}` : ''}:</strong>
                        <p style={{ marginTop: 'var(--space-2)' }}>{infoRequestJustification}</p>
                    </div>
                )}

                <Stepper currentStep={currentStep} />

                {error && <p className="auth-error" style={{ marginBottom: 'var(--space-4)' }}>{error}</p>}

                {currentStep === 0 && <StepPersonalInfo onNext={() => setCurrentStep(1)} />}
                {currentStep === 1 && (
                    <StepEducationalData
                ...
                {currentStep === 2 && userId && (
                    <StepAttachments
                        ...
                        userId={userId}
                        existingFiles={student ? {
                            foto_3x4: student.foto_3x4_path ?? null,
                            comprovante_matricula: student.comprovante_matricula_path ?? null,
                            documento_identidade: student.documento_identidade_path ?? null,
                        } : undefined}
                    />
                )}
            </div>
        ...
        <div className="dashboard-page">
            ...
            {student.status === 'rejected' && (
                <section className="section">
                    {selfCancelled ? (
                        <div className="alert-warning" style={{ marginBottom: 'var(--space-4)' }}>
                            <strong>Você encerrou sua carteirinha.</strong>
                            <p style={{ marginTop: 'var(--space-2)' }}>
                                Caso deseje uma nova carteirinha, clique no botão abaixo para reenviar sua solicitação para análise.
                            </p>
                            <button
                        ...
                        <div className="alert-error" style={{ marginBottom: 'var(--space-4)' }}>
                            ...
                            {rejectionReason && (
                                <p style={{ marginTop: 'var(--space-2)' }}>
                                    Motivo{rejectionExecutive ? ` (${rejectionExecutive})` : ''}: {rejectionReason}
                                </p>
                            )}
                            <p style={{ marginTop: 'var(--space-2)', fontSize: 'var(--text-xs)', color: 'var(--gray-500)' }}>
                                Não é possível enviar uma nova solicitação no momento. Entre em contato para mais informações.
                            </p>
                        </div>
                    )}
                </section>
            ...
            {/* Carteirinha aprovada: frente + verso */}
            {student.status === 'approved' && (
                <section className="section">
                    <h2 className="section-title">Minha Carteirinha</h2>
                    <div className="student-card-duo" ref={cardRef}>
                        {/* FRENTE */}
                        <div className="student-card-document">
                            <div className="student-card-doc-header">
                                <div className="student-card-doc-header-row">
                                    <img src="/logo-prefeitura.png" alt="Logo" className="student-card-doc-logo" crossOrigin="anonymous" />
                                    <div>
                            ...
                            <div className="student-card-doc-body">
                                {photoUrl ? (
                                    <img src={photoUrl} alt="Foto 3x4" className="student-card-doc-photo" crossOrigin="anonymous" />
                                ) : (
                                ...
                                <div className="student-card-doc-fields">
                                    <div className="student-card-doc-field">
                                        ...
                                        <span className="student-card-doc-field-value">{displayName}</span>
                                    </div>
                                    <div className="student-card-doc-field">
                                        <span className="student-card-doc-field-label">CPF:</span>
                                        <span className="student-card-doc-field-value">{user ? formatCPF(user.cpf) : '—'}</span>
                                    </div>
                                    <div className="student-card-doc-field">
                                        <span className="student-card-doc-field-label">Curso:</span>
                                        <span className="student-card-doc-field-value">{studentFull?.cursos?.nome ?? '—'}</span>
                                    </div>
                                    <div className="student-card-doc-field">
                                        <span className="student-card-doc-field-label">Período:</span>
                                        <span className="student-card-doc-field-value">{studentFull?.periodos?.nome ?? '—'}</span>
                                    </div>
                                    <div className="student-card-doc-field">
                                        <span className="student-card-doc-field-label">Instituição:</span>
                                        <span className="student-card-doc-field-value">{studentFull?.instituicoes?.nome ?? '—'}</span>
                                    </div>
                                    <div className="student-card-doc-field">
                                        <span className="student-card-doc-field-label">Matrícula:</span>
                                        <span className="student-card-doc-field-value">{student.student_id_number || '—'}</span>
                                    </div>
                                    <div className="student-card-doc-field">
                                        <span className="student-card-doc-field-label">Cidade:</span>
                                        <span className="student-card-doc-field-value">{studentFull?.cidades?.nome ?? '—'}</span>
                                    </div>
                                    <div className="student-card-doc-field">
                                        <span className="student-card-doc-field-label">Telefone:</span>
                                        <span className="student-card-doc-field-value">{user?.phone ? formatPhone(user.phone) : '—'}</span>
                                    </div>
                                </div>
                            </div>
                            <div className="student-card-doc-footer">
                                <span>Validade: <strong>{cardExpiryDate ? formatDate(cardExpiryDate) : '—'}</strong></span>
                            </div>
                        </div>
                        ...
                        <div className="student-card-document">
                            ...
                            <div className="student-card-doc-back">
                                {qrCodeDataUrl && (
                                    <img src={qrCodeDataUrl} alt="QR Code" className="student-card-doc-qr" />
                                )}
                    ...
                    <div className="student-card-doc-actions">
                        <button
                            className="btn btn-primary btn-sm"
                            onClick={async () => {
                                ...
                                link.download = 'carteirinha-estudantil.png';
                                link.href = dataUrl;
                                link.click();
                            }}
                        >
                        ...
                        <button
                            ...
                            onClick={async () => {
                                ...
                                doc.title = 'Carteirinha Estudantil';
                                const style = doc.createElement('style');
                                style.textContent = '@page { margin: 0; size: auto; } @media print { body { margin: 0; } }';
                                doc.head.appendChild(style);
                                const body = doc.body;
                                body.style.cssText = 'margin:0;padding:0;';
                                const img = doc.createElement('img');
                                img.src = dataUrl;
                                img.style.cssText = 'width:115%;display:block;';
                                body.appendChild(img);
                                img.onload = () => printWin.print();
                            }}
                        >
                            Imprimir / PDF
                        </button>
                    ...
                    <div className="card-settings-section">
                        <h3 className="section-title">Configurações da Carteirinha</h3>
                        <p className="card-settings-desc">Para encerrar sua carteirinha, confirme digitando seu CPF.</p>
                        <div className="card-settings-row">
                            ...
                            <button
                                ...
                                disabled={cancelCpf.replace(/\D/g, '') !== user?.cpf || cancelLoading}
                                onClick={handleCancelCard}
                            >
                                {cancelLoading ? 'Encerrando...' : 'Encerrar minha carteirinha'}
                            </button>
                        </div>
                    </div>
                </section>
            ...
            {student.status === 'pending' && (
                <section className="section">
                    <div className="empty-state card">
                        ...
                        <p className="empty-state-desc">Sua solicitação está sendo analisada.</p>
                ...
                {timeline.length === 0 ? (
                    ...
                    <>
                        <div className="timeline">
                            {(timelineExpanded ? timeline : timeline.slice(0, 4)).map((entry) => (
                                <div key={entry.id} className="timeline-item">
                                    <div className={`timeline-dot ${entry.action}`} />
                                    <div className="timeline-content">
                                        <p className="timeline-action">{actionLabel[entry.action] ?? entry.action}</p>
                                        <p className="timeline-meta">
                                            {entry.users?.full_name ?? 'Sistema'} — {formatDateTime(entry.created_at)}
                                        </p>
                                        {entry.justification && (
                                            <p className="timeline-justification">{entry.justification}</p>
                                        )}
                                    </div>
                                </div>
                            ))}
                        ...
                        {timeline.length > 4 && (
                            ...
                            >
                                {timelineExpanded ? 'Ocultar histórico' : `Visualizar histórico completo (${timeline.length})`}
                            </button>
                        )}
                    </>
                )}
