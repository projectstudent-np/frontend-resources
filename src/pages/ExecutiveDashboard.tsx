...
function JustificationDialog({
    ...
    return (
        <div className="exec-modal-overlay" onClick={onCancel}>
            <div className="justification-dialog" onClick={(e) => e.stopPropagation()}>
...
function TicketDetailModal({
    ...
    async function loadDetails() {
        ...
        if (studentData) {
            ...
            const paths = [
                ...
                { key: 'comprovante_matricula', path: studentData.comprovante_matricula_path },
                { key: 'documento_identidade', path: studentData.documento_identidade_path },
            ];
    ...
    async function handleApprove() {
        ...
        const { data: studentData } = await supabase
            ...
            .eq('user_id', ticket.user_id).single();

        // Buscar validade configurada
        const { data: settings } = await supabase
            .from('app_settings').select('value')
            .eq('key', 'card_expiry_date').maybeSingle();
        const expiryDate = settings?.value ?? '2026-12-31';

        await supabase.from('tickets')
            .update({ status: 'approved', reviewed_by: currentUserId, updated_at: new Date().toISOString() })
            .eq('id', ticket.id);
        ...
        if (studentData) {
            const { data: existing } = await supabase.from('student_cards')
                .select('id').eq('student_id', studentData.id)
    ...
    async function handleReject(justification: string) {
        setActionLoading(true);
        await supabase.from('tickets')
            .update({ status: 'rejected', reviewed_by: currentUserId, updated_at: new Date().toISOString() })
            .eq('id', ticket.id);
        await supabase.from('students')
    ...
    async function handleRequestInfo(justification: string) {
        setActionLoading(true);
        await supabase.from('tickets')
            .update({ status: 'info_requested', reviewed_by: currentUserId, updated_at: new Date().toISOString() })
            .eq('id', ticket.id);
        await supabase.from('students')
    ...
    async function handleReopen() {
        setActionLoading(true);
        await supabase.from('tickets')
            .update({ status: 'open', reviewed_by: null, updated_at: new Date().toISOString() })
            .eq('id', ticket.id);
        await supabase.from('students')
                ...
                <div className="exec-modal" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="exec-modal-header">
                        <div className="exec-modal-title">
                            <span style={{ fontFamily: 'monospace' }}>#{ticket.ticket_number}</span>
                    ...
                    <div className="exec-modal-body">
                        {loadingData ? (
                            ...
                            <>
                                ...
                                <div className="exec-modal-section">
                                    ...
                                    <div className="exec-modal-grid">
                                        <div className="exec-modal-field">
                                            <span className="exec-modal-field-label">Nome completo</span>
                                            <span className="exec-modal-field-value">{ticket.users?.full_name ?? '—'}</span>
                                        </div>
                                        <div className="exec-modal-field">
                                            <span className="exec-modal-field-label">CPF</span>
                                            <span className="exec-modal-field-value">{ticket.users?.cpf ? formatCPF(ticket.users.cpf) : '—'}</span>
                                        </div>
                                        <div className="exec-modal-field">
                                            <span className="exec-modal-field-label">Email</span>
                                            <span className="exec-modal-field-value">{ticket.users?.email ?? '—'}</span>
                                        </div>
                                        <div className="exec-modal-field">
                                            <span className="exec-modal-field-label">Telefone</span>
                                            <span className="exec-modal-field-value">{ticket.users?.phone ? formatPhone(ticket.users.phone) : '—'}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {student && (
                                    <div className="exec-modal-section">
                                        <h3 className="exec-modal-section-title">Dados Educacionais</h3>
                                        <div className="exec-modal-grid">
                                            <div className="exec-modal-field">
                                                <span className="exec-modal-field-label">Curso</span>
                                                <span className="exec-modal-field-value">{student.cursos?.nome ?? '—'}</span>
                                            </div>
                                            <div className="exec-modal-field">
                                                <span className="exec-modal-field-label">Período</span>
                                                <span className="exec-modal-field-value">{student.periodos?.nome ?? '—'}</span>
                                            </div>
                                            <div className="exec-modal-field">
                                                <span className="exec-modal-field-label">Instituição</span>
                                                <span className="exec-modal-field-value">{student.instituicoes?.nome ?? '—'}</span>
                                            </div>
                                            <div className="exec-modal-field">
                                                <span className="exec-modal-field-label">Cidade da Faculdade</span>
                                                <span className="exec-modal-field-value">{student.cidades?.nome ?? '—'}</span>
                                            </div>
                                            <div className="exec-modal-field">
                                                <span className="exec-modal-field-label">Matrícula / RA</span>
                                                <span className="exec-modal-field-value">{student.student_id_number || '—'}</span>
                                            </div>
                                        </div>
                                    </div>
                                )}
                                ...
                                <div className="exec-modal-section">
                                    ...
                                    <div className="exec-docs-grid">
                                        {[
                                            { key: 'foto_3x4', label: 'Foto 3x4', path: student?.foto_3x4_path },
                                            { key: 'comprovante_matricula', label: 'Comprovante de Matrícula', path: student?.comprovante_matricula_path },
                                            { key: 'documento_identidade', label: 'Documento de Identidade', path: student?.documento_identidade_path },
                                        ].map(({ key, label, path }) => (
                                            <div key={key} className="exec-doc-item">
                                                {!path ? (
                                                    ...
                                                    <a href={docUrls[key]} target="_blank" rel="noopener noreferrer" className="exec-doc-link">
                                                    ...
                                                    <div className="dashboard-loading" style={{ minHeight: 80 }} />
                                                )}
                                            ...
                                            {history.map((h) => (
                                                <div key={h.id} className="timeline-item">
                                                    ...
                                                    <div className="timeline-content">
                                                        <p className="timeline-action">{actionLabel[h.action] ?? h.action}</p>
                                                        <p className="timeline-meta">
                                                            {h.users?.full_name ?? 'Sistema'} — {formatDateTime(h.created_at)}
                                                        </p>
                                                        {h.justification && (
                                                            <p className="timeline-justification">{h.justification}</p>
                                                        )}
                                                    </div>
                                                </div>
                                            ))}
                    ...
                    {/* Carteirinha oculta para exportação */}
                                    ...
                                    <div className="student-card-doc-body">
                                        ...
                                        <div className="student-card-doc-fields">
                                            <div className="student-card-doc-field">
                                                ...
                                                <span className="student-card-doc-field-value">{ticket.users?.full_name ?? '—'}</span>
                                            </div>
                                            <div className="student-card-doc-field">
                                                <span className="student-card-doc-field-label">CPF:</span>
                                                <span className="student-card-doc-field-value">{ticket.users?.cpf ? formatCPF(ticket.users.cpf) : '—'}</span>
                                            </div>
                                            <div className="student-card-doc-field">
                                                <span className="student-card-doc-field-label">Curso:</span>
                                                <span className="student-card-doc-field-value">{student.cursos?.nome ?? '—'}</span>
                                            </div>
                                            <div className="student-card-doc-field">
                                                <span className="student-card-doc-field-label">Período:</span>
                                                <span className="student-card-doc-field-value">{student.periodos?.nome ?? '—'}</span>
                                            </div>
                                            <div className="student-card-doc-field">
                                                <span className="student-card-doc-field-label">Instituição:</span>
                                                <span className="student-card-doc-field-value">{student.instituicoes?.nome ?? '—'}</span>
                                            </div>
                                            <div className="student-card-doc-field">
                                            ...
                                            <div className="student-card-doc-field">
                                                ...
                                                <span className="student-card-doc-field-value">{student.cidades?.nome ?? '—'}</span>
                                            </div>
                                            <div className="student-card-doc-field">
                                                <span className="student-card-doc-field-label">Telefone:</span>
                                                <span className="student-card-doc-field-value">{ticket.users?.phone ? formatPhone(ticket.users.phone) : '—'}</span>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div className="exec-modal-footer">
                            {ticket.status === 'open' && (
                                <>
                                    ...
                                    <button className="btn btn-danger btn-sm" onClick={() => setJustDialog('reject')} disabled={actionLoading}>
                                        Rejeitar
                                    </button>
                                    <button className="btn btn-secondary btn-sm" onClick={() => setJustDialog('info')} disabled={actionLoading}>
                                        Mais Infos
                                    </button>
                                </>
                            )}
                            {ticket.status === 'approved' && (
                                <>
                                    <div style={{ position: 'relative' }}>
                                        <button className="btn btn-primary btn-sm" onClick={() => setDownloadMenu(!downloadMenu)}>
                                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                            ...
                                            <div className="exec-download-menu">
                                                <button
                                                    ...
                                                    onClick={async () => {
                                                        ...
                                                        link.download = `carteirinha-${ticket.users?.full_name ?? 'estudante'}.png`;
                                                        link.href = dataUrl;
                                                        link.click();
                                    ...
                                    <button className="btn btn-secondary btn-sm" onClick={handleReopen} disabled={actionLoading}>
                            ...
                            {ticket.status === 'rejected' && (
                                <button className="btn btn-secondary btn-sm" onClick={handleUnblock} disabled={actionLoading}>
            ...
            if (activeTab === 'self_cancelled' || activeTab === 'rejected') {
                ...
                if (ticketIds.length > 0) {
                    ...
                    if (activeTab === 'self_cancelled') {
                        result = result.filter(t => latestPerformer.get(t.id) === t.user_id);
                    } else {
                        result = result.filter(t => latestPerformer.get(t.id) !== t.user_id);
                    }
                }
        ...
        <div className="dashboard-page">
            <div className="dashboard-header">
                <div>
                    ...
                    <p className="dashboard-sub">Gerencie solicitações de carteirinha dos estudantes</p>
                </div>
            </div>
            ...
            <div className="admin-toolbar">
                ...
                <span className="admin-count">{filtered.length} ticket{filtered.length !== 1 ? 's' : ''}</span>
            </div>
            ...
            {loading ? (
                ...
                <div className="empty-state card">
                    ...
                    <h2 className="empty-state-title">
                        {search ? 'Nenhum resultado encontrado' : 'Nenhum ticket neste status'}
                    </h2>
                        ...
                        >
                            <div className="exec-ticket-header">
                                <div>
                                    <p className="exec-ticket-user">
                                        <span className="exec-ticket-number">#{ticket.ticket_number}</span>
                                        {' '}
                                        {ticket.users?.full_name ?? 'Desconhecido'}
                                    </p>
                            ...
                            <div className="exec-ticket-info">
                                <span className="ticket-type">
                                    {ticket.type === 'card_creation' ? 'Solicitação de Carteirinha' : 'Pergunta'}
                                </span>
